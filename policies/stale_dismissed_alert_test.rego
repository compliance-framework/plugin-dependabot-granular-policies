...
test_skip_reason_set_for_open_alert if {
	sr := dependabot_granular_stale_dismissed_alert.skip_reason with input as [object.union(mock_alert, {"state": "open"})]
	sr == "Alert state is open, this policy only applies to dismissed alerts"
}

test_skip_reason_set_for_fixed_alert if {
	sr := dependabot_granular_stale_dismissed_alert.skip_reason with input as [object.union(mock_alert, {"state": "fixed"})]
	sr == "Alert state is fixed, this policy only applies to dismissed alerts"
}

test_skip_reason_not_set_for_dismissed_alert if {
	not dependabot_granular_stale_dismissed_alert.skip_reason with input as [mock_alert]
}
